...
| **2. Tool Interaction** | **4** / 5 | Hệ thống không thể dùng tri thức tĩnh có sẵn (parametric knowledge) mà bắt buộc phải kết nối 2 chiều với hệ thống WMS/ERP qua MCP Server: Tool đọc (`query_shipment` / `get_inventory_location`) để lấy dữ liệu thời gian thực và Tool ghi (`update_order_status`) để cập nhật trạng thái vào cơ sở dữ liệu. |
| **3. Dynamic Decision** | **4** / 5 | Nhánh xử lý tiếp theo phụ thuộc hoàn toàn vào kết quả quan sát (Observation) từ Tool ở bước trước: Nếu mã vận đơn không tồn tại thì xử lý ngoại lệ; Nếu đơn hàng đang bị khóa/hư hại (Hold/Damaged) thì chuyển luồng thông báo sự cố; Nếu hợp lệ thì tiến hành cập nhật trạng thái mới. |
| **4. Long Horizon Goal** | **3** / 5 | Agent cần duy trì mục tiêu xử lý đơn hàng xuyên suốt chuỗi hội thoại đa lượt (multi-turn): từ tra cứu vị trí, hỏi xác nhận thông tin bổ sung từ nhân sự kho (Human-in-the-loop), đến thực thi cập nhật và lưu vết audit log thành công. |
| **TỔNG ĐIỂM AGENTIC FIT** | **16 / 20** | *Kết luận: 16/20 >> 12/20. Bài toán phù hợp để phát triển ReAct Agent tích hợp MCP Server.* |
